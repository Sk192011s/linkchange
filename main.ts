// main.ts (Final Range Request & Status Code Fix)
...
console.log("Secure Force Download Proxy (Status Code Fix) is starting...");
...
async function handler(req: Request): Promise<Response> {
    ...
    if (downloadPattern.exec(url)) {
        ...
        if (token !== DOWNLOAD_TOKEN) {
            return new Response("Access Denied: Invalid download token.", { status: 403 });
        }
        ...
        try {
            ...
            // Pass through all essential headers from the original source
            ...
            // Determine if it should be played inline or downloaded
            const shouldPlayInline = searchParams.get("play") === "true";
            
            responseHeaders.set('Content-Type', 'video/mp4'); // Assume mp4 for simplicity
            responseHeaders.set('Content-Disposition', shouldPlayInline ? 'inline' : `attachment; filename="${filename}"`);
            // --- END OF FINAL FIX ---

            return new Response(videoResponse.body, { 
                status: videoResponse.status, // CRITICAL: Pass through original status (200 or 206)
...
function getAdminPageHTML(videos: any[], token: string, generatedLink: string): string {
    const generatedLinkHTML = generatedLink ? `<div class="result-box"><h3>Generated Link (Play & Download):</h3><p>Use the link below. Add <b>&play=true</b> at the end to stream in browser.</p><input type="text" id="generated-link-input" value="${decodeURIComponent(generatedLink)}" readonly><button onclick="copyLink()">Copy Base Link</button></div>` : '';
