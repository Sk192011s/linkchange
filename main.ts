// main.ts (Final Secure "Force Download" Version)
import { serve } from "https://deno.land/std@0.224.0/http/server.ts";

const kv = await Deno.openKv();

// --- TWO SEPARATE TOKENS FOR SECURITY ---
const ADMIN_TOKEN = Deno.env.get("ADMIN_TOKEN") || "fallback-admin-token";
const DOWNLOAD_TOKEN = Deno.env.get("DOWNLOAD_TOKEN") || "fallback-download-token"; // This is the public token for links

console.log("Secure Force Download Proxy Server is starting...");
...
async function handler(req: Request): Promise<Response> {
    ...
    if (pathname === "/admin") {
        // Admin panel only checks for ADMIN_TOKEN
        if (searchParams.get("token") !== ADMIN_TOKEN) return new Response("Forbidden: Invalid Admin Token.", { status: 403 });
        const videos: any[] = [];
    ...
    if (pathname === "/generate" && method === "POST") {
        ...
        // This action is protected by ADMIN_TOKEN
        ...
        // --- THE FIX: Use the public DOWNLOAD_TOKEN for the generated link ---
        const generatedLink = `${url.origin}/download/${fileSlug}?token=${DOWNLOAD_TOKEN}`;
    ...
    if (downloadPattern.exec(url)) {
        ...
        // Download endpoint only checks for the public DOWNLOAD_TOKEN
        if (token !== DOWNLOAD_TOKEN) {
            return new Response("Access Denied: Invalid or missing download token.", { status: 403 });
        }
        ...
        try {
            ...
            if (!videoResponse.ok || !videoResponse.body) return new Response("Failed to fetch the file from the source.", { status: videoResponse.status });
...
function getAdminPageHTML(videos: any[], token: string, generatedLink: string): string {
    const generatedLinkHTML = generatedLink ? `<div class="result-box"><h3>Generated App Download Link:</h3><input type="text" id="generated-link-input" value="${decodeURIComponent(generatedLink)}" readonly><button onclick="copyLink()">Copy</button></div>` : '';
    ...
    <script>function copyLink(){const i=document.getElementById('generated-link-input');i.select();i.setSelectionRange(0,99999);navigator.clipboard.writeText(i.value).then(()=>{alert('Link copied!')});}</script>
    </body></html>`;
}
